 ...
 $model = new Model(); foreach($users_list as $k=>$v){ ?>
                    ...
                    <td colspan="10" style="padding:0px;">
                        ...
                        <td >
                            ...
                            <?php
 for ($i=1;$i<13;$i++){ $map = array(); if ($i<10){ $tmp = "0".$i; }else{ $tmp = $i; } $this_year_month = $the_year."-".$tmp; $map = array(); $map['role_id'] = $v['user_id']; $map['the_year_month'] = $this_year_month; $log_month_count_1 = $model->table("jl_log")->where($map)->count('distinct(the_date)'); $days_month = date("t",strtotime($this_year_month)); $days_year = 365; $map = array(); $map['role_id'] = $v['user_id']; $map['the_year_month'] = $this_year_month; $log_month_count_2 = $model->table("jl_log")->where($map)->count('distinct(the_date)'); $map = array(); $map['role_id'] = $v['user_id']; $map['the_year_month'] = $this_year_month; $log_month_count_3 = $model->table("jl_log")->where($map)->count(); ?>
                            ...
                            <?php
 $map = array(); $map['role_id'] = $v['user_id']; $map['the_year'] = $the_year; $log_year_count_1 = $model->table("jl_log")->where($map)->count('distinct(the_date)'); $log_year_count_2 = $days_year - $log_year_count_1; $map = array(); $map['role_id'] = $v['user_id']; $map['the_year'] = $the_year; $log_year_count_3 = $model->table("jl_log")->where($map)->count(); } ?>
                            </div>
                        </td>
                        <td width="200">
                            <ul class="tb_year">
                            <li><b>全年合计</b></li>
                            <li><?php echo ($log_year_count_1); ?><!--本年度--></li>
                            <li><?php echo ($log_year_count_2); ?></li>
                            <li class="noborder"><?php echo ($log_year_count_3); ?></li>
                            </ul>
                        </td>    
                        </tr> 
                        </table>
