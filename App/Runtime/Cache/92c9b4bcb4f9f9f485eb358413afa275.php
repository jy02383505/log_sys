		...
		<div class="span12">
			<ul class="nav pull-left">
				...
				<li class="pull-left">
                    ...
                    <input type="hidden" name="m" value="Chuchai"/><!--模型：出差-->
					...
					<ul class="nav pull-left">
                        ...
                        <li class="pull-left">
                         ...
                         <li class="pull-left" style="padding-left:10px;display:none;">
                        	<button type="button" class="btn" name="btn_tj" id="btn_tj">  统计</button>
                        </li>                       
			...
			<div class="pull-right">
            	<a id="add_kaoqin_row" href="<?php echo U('Chuchai/add');?>" class="btn btn-primary" style="margin-right:20px;"><i class="icon-plus"></i> &nbsp;增加出差记录</a>
                ...
                <?php if($show_type == 0): ?><table class="table table-hover table-striped table_thead_fixed"> 
					...
					<tbody>
						<?php if(is_array($datas)): $i = 0; $__LIST__ = $datas;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><tr>
                            ...
                            <td>
                                <?php switch($vo["type"]): case "1": ?>省内<?php break;?>
                                    <?php case "2": ?>省外<?php break;?>
                                    <?php case "3": ?>一线<?php break;?>
                                    <?php default: ?>0<?php endswitch;?>
                            </td>
                            <td><?php echo ($vo["chuchai_days"]); ?>天</td>
							...
							<?php  if (1 == session('user_id') || 2 == session('position_id')){ ?>
                            	<?php  if (!$vo['checked']){ ?>
                                	<a href="<?php echo U('Chuchai/edit', 'id='.$vo['id']);?>">编缉</a> &nbsp;
									<a href="<?php echo U('Chuchai/accept', 'id='.$vo['id']);?>"><font color='red'>待审核</font></a> &nbsp;
                                    <a href="<?php echo U('Chuchai/delete', 'id='.$vo['id']);?>" onclick="return confirm('删除后无法恢复,确定要删除吗')">删除</a>
                                <?php
 } else { ?>
                                	<font color='#999999'>编缉</font> &nbsp;
                                	<a href="<?php echo U('Chuchai/accept', 'id='.$vo['id']);?>">已审核</a> &nbsp;
                                <?php
 } ?>
							<?php
 } else { if (!$vo['checked']){ ?>
...
<script type="text/javascript" src="../Chucai/__PUBLIC__/js/jquery-1.9.0.min.js"></script>
<script type="text/javascript" src="../Chucai/__PUBLIC__/js/jquery.validate.min.js"></script>
<script type="text/javascript" src="../Chucai/__PUBLIC__/js/WdatePicker.js"></script>
...
$(document).ready(function() {
	...
	$('#btn_tj').click(function(){
		//url
		var url_tj = "<?php echo U('Chuchai/tj');?>";
		//select value
