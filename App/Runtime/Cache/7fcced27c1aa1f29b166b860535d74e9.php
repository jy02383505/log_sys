...
<div class="container">
	...
	<div class="page-header">
		<h4>请假管理</h4>
        <?php if($_SESSION['admin']== '1'): ?><div class="yv_btns pull-right" style="margin-bottom:15px;">
                <a href='<?php echo U("YearVacation/index");?>' class="btn btn-sm btn-info">年假列表</a>
                <a href='<?php echo U("YearVacation/add");?>' class="btn btn-sm btn-success">年假设置</a>
            </div><?php endif; ?>
	</div>
	...
	<div class="row">
		<div class="span12">
			...
			<div class="pull-right">
            	<a id="add_kaoqin_row" href="<?php echo U('Qinjia/add');?>" class="btn btn-primary" style="margin-right:20px;"><i class="icon-plus"></i> &nbsp;增加请假记录</a>
                ...
                <?php if($show_type == 0): ?><table class="table table-hover table-striped table_thead_fixed"> 
					...
					<tbody>
						<?php if(is_array($datas)): $i = 0; $__LIST__ = $datas;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><tr>
                            ...
                            <td>
                                <?php switch($vo["type"]): case "1": ?>事假<?php break;?>
                                    <?php case "2": ?>病假<?php break;?>
                                    <?php case "3": ?>调休<?php break;?>
                                    <?php default: ?>0<?php endswitch;?>
                            </td>
                            <td><?php echo ($vo["hour_num"]); ?></td>
                            <td><?php echo ($vo["shijianduan_ch"]); ?></td>
                            <td>
                                <?php  $weekArr=array("星期日","星期一","星期二","星期三","星期四","星期五","星期六"); $week_num = date("w",strtotime($vo[the_date])); echo $weekArr[$week_num]; ?>
                            </td>
							...
							<?php  if (1 == session('user_id') || 2 == session('position_id')){ ?>
                            	<?php  if (!$vo['checked']){ ?>
                                	<a href="<?php echo U('Qinjia/edit', 'id='.$vo['id']);?>">编缉</a> &nbsp;
									<a href="<?php echo U('Qinjia/accept', 'id='.$vo['id']);?>">待审核</a> &nbsp;
                                    <a href="<?php echo U('Qinjia/delete', 'id='.$vo['id']);?>" onClick="return confirm('确认要删除？');" >删除</a>
                                <?php
 } else { ?>
                                	<font color='#999999'>编缉</font> &nbsp;
                                	<a href="<?php echo U('Qinjia/accept', 'id='.$vo['id']);?>">已审核</a> &nbsp;
                                <?php
 } ?>
							<?php
 } else { ?>
...
$(document).ready(function() {
	...
	$('#btn_tj').click(function(){
		//url
		var url_tj = "<?php echo U('Qinjia/tj');?>";
		//select value
