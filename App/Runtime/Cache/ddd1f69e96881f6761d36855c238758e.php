...
<div class="container">
 ...
 $model = new Model(); foreach($users_list as $k=>$v){ ?>
                    ...
                    [<?php echo ($v['name']); ?>] 统计：（<?php echo ($the_year); ?>年）
                    </td>
                    </tr>
                    ...
                    <td colspan="10" style="padding:0px;">
                        ...
                        <td >
                            ...
                            <?php
 for ($i=1;$i<13;$i++){ $map = array(); if ($i<10){ $tmp = "0".$i; }else{ $tmp = $i; } $this_year_month = $the_year."-".$tmp; $map = array(); $map['role_id'] = $v['user_id']; $map['the_year_month'] = $this_year_month; $log_month_count_1 = $model->table("jl_log")->where($map)->count('distinct(the_date)'); $days_month = date("t",strtotime($this_year_month)); $map = array(); $map['role_id'] = $v['user_id']; $map['the_year_month'] = $this_year_month; $log_month_count_2 = $model->table("jl_log")->where($map)->count('distinct(the_date)'); $map = array(); $map['role_id'] = $v['user_id']; $map['the_year_month'] = $this_year_month; $log_month_count_3 = $model->table("jl_log")->where($map)->count(); ?>
                            
                                
                                    <ul class="tb_month">
                                    <dt><?php echo $i;?>月</dt>
                                    <dt><?php echo $log_month_count_1;?></dt>
                                    <dt><?php echo $days_month - $log_month_count_2;?></dt>
                                    <dt class="noborder"><?php echo $log_month_count_3;?></dt>
