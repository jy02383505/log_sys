...
<div class="container">
 ...
 <style>                  
	...
	.tb_left li.noborder{clear:both;height:30px;line-height:30px;border:none;}
 	...
 	.tb_month{float:left;width:60px;height:210px;margin:0px;padding:0px;}
 ...
 $model = new Model(); foreach($users_list as $k=>$v){ ?>
                    ...
                    [<?php echo ($v['name']); ?>] 考勤统计：（<?php echo ($the_year); ?>年）
                    </td>
                    </tr>
                    ...
                    <td colspan="10" style="padding:0px;">
                        ...
                        <td >
                            ...
                            <?php
 $cfgKqModel = D("Configyearmonthdaytime"); for ($i=1;$i<13;$i++){ $map = array(); if ($i<10){ $dmonth = "0".$i; }else{ $dmonth = $i; } $this_year_month = $the_year."-".$dmonth; $map = array(); $map['user_id'] = $v['user_id']; $map['sign_month'] = $this_year_month; $map['type'] = 0; $map['cdzt_minutes'] = array('gt','0'); $chidao_num = $model->table("jl_kaoqin")->where($map)->count(); $map = array(); $map['user_id'] = $v['user_id']; $map['sign_month'] = $this_year_month; $map['type'] = 1; $map['cdzt_minutes'] = array('gt','0'); $zhaotui_num = $model->table("jl_kaoqin")->where($map)->count(); $map = array(); $map['year'] = $the_year; $map['month'] = $dmonth; $map['state'] = 1; $count_yinqiandao_num = $cfgKqModel->where($map)->count(); $map = array(); $map['user_id'] = $v['user_id']; $map['sign_month'] = $this_year_month; $map['type'] = 0; $qiandao_num = $model->table("jl_kaoqin")->where($map)->count(); $map = array(); $map['user_id'] = $v['user_id']; $map['sign_month'] = $this_year_month; $map['type'] = 1; $qiantui_num = $model->table("jl_kaoqin")->where($map)->count(); ?>
                            
                                
                                    <ul class="tb_month">
                                    <dt><?php echo $i;?>月</dt>
                                    <dt><?php echo $chidao_num;?></dt>
                                    <dt><?php echo $zhaotui_num;?></dt>
                                    <dt><?php echo $count_yinqiandao_num;?></dt>
                                    <dt><?php echo $qiandao_num;?></dt>
                                    <dt><?php echo $qiantui_num;?></dt>
                                    <dt class="noborder"><a href="<?php echo U("count/count_one_people_days","userid=".$v['user_id']."&the_year=".$the_year."&the_month=".$dmonth);?>">详细</a></dt>
                                    </ul>
                                
                            <?php
 $map = array(); $map['user_id'] = $v['user_id']; $map['sign_year'] = $the_year; $map['type'] = 0; $map['cdzt_minutes'] = array('gt','0'); $year_chidao_num = $model->table("jl_kaoqin")->where($map)->count(); $map = array(); $map['user_id'] = $v['user_id']; $map['sign_year'] = $the_year; $map['type'] = 1; $map['cdzt_minutes'] = array('gt','0'); $year_zhaotui_num = $model->table("jl_kaoqin")->where($map)->count(); } ?>
                            </div>
                        </td>
                        <td width="200">
                            <ul class="tb_year">
                            <li><b>全年合计</b></li>
                            <li><?php echo ($year_chidao_num); ?><!--本年度--></li>
                            <li><?php echo ($year_zhaotui_num); ?></li>
                ...
                <?php
                    ...
                    <tr>
                    <td colspan="17">
                    应签到/签退总次数，指需要正常签到/签退的总次数<br>
                    全天只打一次卡，均被作为签到记录处理<br>
                    迟到次数，指有打卡的情况，如果全天未打卡，则不计算在内。（早退也同样只针对有打卡记录的）<br>
