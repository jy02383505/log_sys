...
<div class="container">
	...
	<div class="page-header" style="border:none; font-size:14px; ">
		<ul class="nav nav-tabs">
		  ...
		  <li><a href="<?php echo U('customer/analytics');?>"><img src="__PUBLIC__/img/analytics_icon.png"/> &nbsp;<?php echo L('THE_CUSTOMER_STATISTICS');?></a></li>
	...
	<?php if($_GET['content'] != 'resource'): ?><p class="view">
		<b><img src="__PUBLIC__/img/view.png"/>  <?php echo L('UNIFIED_CUSTOMER_VIEW');?></b>
		...
		<a href="<?php echo U('customer/index','by=add');?>" <?php if($_GET['by']== 'add'): ?>class="active"<?php endif; ?>><?php echo L('RECENTLY_CREATED');?></a> | 
		<a href="<?php echo U('customer/index','by=update');?>" <?php if($_GET['by']== 'update'): ?>class="active"<?php endif; ?>><?php echo L('recent_updates');?></a> &nbsp;  &nbsp; 
			...
			<ul class="nav pull-left">
				<?php if($_SESSION['admin']== 1 or $_GET['by']!= 'deleted'): ?><div class="btn-group pull-left">
						...
						<ul class="dropdown-menu">
							<li><a id="delete"  style="margin-right: 5px;" href="javascript:void(0)"><?php echo L('BATCH_REMOVE');?></a></li>
							...
							<li><a id="remove"  style="margin-right: 5px;" href="javascript:void(0)"><?php echo L('BATCH_INTO_THE_CUSTOMER_POOL');?></a></li><?php endif; ?>
						</ul>
					</div><?php endif; ?>
				<li class="pull-left">
					<form class="form-inline" id="searchForm" action="index.php" method="get">
						...
						<li class="pull-left">
							<div class="btn-group">
								<a class="btn dropdown-toggle" data-toggle="dropdown" href="#">
									<i class="icon-comment" style="color:rgb(107, 168, 192);"></i>&nbsp;<?php echo L('SEND_SMS');?>
									<span class="caret"></span>
								</a>
							...
							<div class="btn-group">
								<a class="btn dropdown-toggle" data-toggle="dropdown" href="#">
									<i class="icon-envelope" style="color:rgb(107, 168, 192);"></i>&nbsp;<?php echo L('SEND_EMAIL');?>
									<span class="caret"></span>
								</a>	
								<ul class="dropdown-menu">
			...
			<div class="pull-right">
				...
				<div class="btn-group">
					...
					<ul class="dropdown-menu">
						...
						<li><a href="<?php echo U('customer/excelExport');?>" id="excelExport"  class="link"> <i class="icon-download"> </i> <?php echo L('EXPORT_CUSTOMERS');?></a></li>
					</ul>
				</div>
			</div>
		</div>
				...
				<table class="table table-hover table-striped table_thead_fixed">
					...
					<thead>
						<tr id="childNodes_num">
							...
							<?php if((C('ismobile') != 1) and ($_GET['by']== 'deleted')): ?><th><?php echo L('DELETE_THE_PEOPLE');?></th><th><?php echo L('DELETE_THE_TIME');?></th>
							<?php elseif(C('ismobile') != 1): ?><th><?php echo L('CREATION_TIME');?></th><?php endif; ?>
							<?php if($_GET['content']!= 'resource' && $_GET['by']!= 'deleted'): ?><th><?php echo L('FROM_THE_DUE_DAY');?></th><?php endif; ?>
					...
					<tbody>
						<?php if(is_array($customerlist)): $i = 0; $__LIST__ = $customerlist;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><tr>
								...
								<?php elseif(C('ismobile') != 1): ?><td><?php echo (date('Y-m-d',$vo["create_time"])); ?></td><?php endif; ?>
								<?php if($_GET['content']!= 'resource' && $_GET['by']!= 'deleted'): ?><td>
										<?php if($vo['days'] <= 7): ?><font color="red"><?php echo ($vo["days"]); echo L('DAY');?></font><?php else: ?><font color="blue"><?php echo ($vo["days"]); echo L('DAY');?></font><?php endif; ?>
									</td><?php endif; ?>			
								<td><a href="<?php echo U('customer/view', 'id='.$vo['customer_id']);?>"><?php echo L('CHECK');?></a>&nbsp;
									<?php if($_GET['by']== 'deleted'): ?><a href="<?php echo U('customer/revert', 'id=' . $vo['customer_id']);?>"><?php echo L('REDUCTION');?></a>&nbsp;
									<?php else: ?><a href="<?php echo U('customer/edit', 'id='.$vo['customer_id']);?>"><?php echo L('COMPILE');?></a><?php endif; ?>
									<?php if($_GET['content']== 'resource'): ?><a rel="<?php echo ($vo['customer_id']); ?>" class="fenpei" href="javascript:void(0)"><?php echo L('DISTRIBUTION');?></a> &nbsp;<a href="<?php echo U('customer/receive', 'customer_id='.$vo['customer_id']);?>"><?php echo L('RECEIVE');?></a><?php endif; ?>
								</td>
...
<div class="hide" id="dialog-assign" title="<?php echo L('CLUES_TO_THE_DISTRIBUTION');?>">loading...</div>
<script type="text/javascript">
