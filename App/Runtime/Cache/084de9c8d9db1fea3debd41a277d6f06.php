...
<div class="container">
 ...
 $model = new Model(); foreach($users_list as $k=>$v){ ?>
                    ...
                    [<?php echo ($v['name']); ?>] 请假统计：（<?php echo ($the_year); ?>年）
                    </td>
                    </tr>
                    ...
                    <td colspan="10" style="padding:0px;">
                        ...
                        <td >
                            ...
                            <?php
 for ($i=1;$i<13;$i++){ $map = array(); if ($i<10){ $tmp = "0".$i; }else{ $tmp = $i; } $this_year_month = $the_year."-".$tmp; $map = array(); $map['user_id'] = $v['user_id']; $map['the_month'] = $this_year_month; $map['type'] = 1; $map['checked'] = 1; $month_chucai_hours_1 = $model->table("jl_kaoqin_qinjia")->where($map)->sum('hour_num'); $month_chucai_hours_1 = round($month_chucai_hours_1/8,1); $map = array(); $map['user_id'] = $v['user_id']; $map['the_month'] = $this_year_month; $map['type'] = 2; $map['checked'] = 1; $month_chucai_hours_2 = $model->table("jl_kaoqin_qinjia")->where($map)->sum('hour_num'); $month_chucai_hours_2 = round($month_chucai_hours_2/8,1); $map = array(); $map['user_id'] = $v['user_id']; $map['the_month'] = $this_year_month; $map['type'] = 3; $map['checked'] = 1; $month_chucai_hours_3 = $model->table("jl_kaoqin_qinjia")->where($map)->sum('hour_num'); $month_chucai_hours_3 = round($month_chucai_hours_3/8,1); ?>
                            ...
                            <?php
 $map = array(); $map['user_id'] = $v['user_id']; $map['the_year'] = $the_year; $map['type'] = 1; $map['checked'] = 1; $year_chucai_hours_1 = $model->table("jl_kaoqin_qinjia")->where($map)->sum('hour_num'); $year_chucai_hours_1 = round($year_chucai_hours_1,1); $map = array(); $map['user_id'] = $v['user_id']; $map['the_year'] = $the_year; $map['type'] = 2; $map['checked'] = 1; $year_chucai_hours_2 = $model->table("jl_kaoqin_qinjia")->where($map)->sum('hour_num'); $year_chucai_hours_2 = round($year_chucai_hours_2,1); $map = array(); $map['user_id'] = $v['user_id']; $map['the_year'] = $the_year; $map['type'] = 3; $map['checked'] = 1; $year_chucai_hours_3 = $model->table("jl_kaoqin_qinjia")->where($map)->sum('hour_num'); $year_chucai_hours_3 = round($year_chucai_hours_3,1); } ?>
                            </div>
                        </td>
                        <td width="200">
                            <ul class="tb_year">
