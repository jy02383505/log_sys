...
<div class="container">
    ...
    <a href="<?php echo U('YearVacation/add');?>"><button type="button" class="btn btn-sm btn-success pull-right add_out">新增年假记录</button></a>
    <table class="table table-hover table-striped table-bordered">
        ...
        <?php if(is_array($rows)): $i = 0; $__LIST__ = $rows;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$row): $mod = ($i % 2 );++$i;?><tr>
                ...
                <td><?php echo ($row[year_vacation]); ?></td>
                <td><?php echo ($row[username]); ?></td>
                <td><?php echo (date("Y-m-d H:i:s", $row[update_time])); ?></td>
                <td>
                    <a href=<?php echo U('YearVacation/edit', "id=$row[id]");?> class="btn btn-sm btn-warning">修改</a>
                    <button type="button" data-href=<?php echo U('YearVacation/delete', "id=$row[id]");?> class="btn btn-sm btn-danger del_it">删除</button>
                </td>
