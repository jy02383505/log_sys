...
class MessageAction extends Action{
	...
	public function send(){
		if($_POST['submit']){
			...
			if(is_array($to_role)){
				...
				alert('success',L('SEND_SUCCESS'),$_SERVER['HTTP_REFERER']);
			}else{
		...
		if($this->isPost()){
			...
			if ('' == $message_id) { 
				...
				if($_GET['model'] == 'receive'){
					...
					alert('success',L('DELETE_SUCCESS'),$_SERVER['HTTP_REFERER']);
	...
	public function tips(){
		...
		$m_task = M('Task');
		$new_num[task] = $m_task->where('owner_role_id = %d and isclose = 0 and status <> "'.L('COMPLETE').'" and is_deleted <> 1', session('role_id'))->count();
