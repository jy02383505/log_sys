...
class KnowledgeAction extends Action{
	...
	public function edit(){
		if($this->isPost()){
			...
			if($knowledge->create()){
				...
				if($knowledge->save()){
					if($_POST['submit'] == L('SAVE')) {
						alert('success', L('ARTICLE_SAVE_SUCCESS'), U('knowledge/index'));
					} else {
			...
			if (!session('?admin')) {
				foreach ($knowledge_idarray as $v) {
					if (!$knowledge->where('knowledge_id = %d and role_id = %d', $v, session('role_id'))->find()){
						alert('error', L('DONOT_HAVE_PERMISSIONS_ONLY_AUTHOR_CAN_DELETE'),U('knowledge/index'));
					}
				}
			}
	...
	public function excelExport(){
		...
		$objActSheet->setCellValue('B1', L('类别'));
