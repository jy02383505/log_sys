...
class BusinessAction extends Action{
	...
	public function add(){
		if($this->isPost()){
			...
			if($m_business->create() && $m_business_data->create()!==false){
				...
				alert('error', $m_business->getError().$m_business_data->getError(),U('business/add'));
			}
		}else{
			$alert = parseAlert();
			$this->alert = $alert;
	...
	public function edit(){
		...
		if($this->isPost()){
			...
			if($m_business->create() && $m_business_data->create()!==false){
				...
				alert('error', $m_business->getError().$m_business_data->getError(),U('business/edit','id='.$business['business_id']));
			}
		}else{
		...
		if ($this->isPost()) {
			...
			if ('' == $business_ids) {
				...
				if($m_business->where('business_id in (%s)', $business_ids)->delete() && $m_business_data->where('business_id in (%s)', $business_ids)->delete()){
					...
					alert('error', L('DELETE_FAILED'), U('business/index'));
				}
			}
		} elseif($_GET['id']) {
	...
	**/
	public function analytics(){
		...
		$money_where = array(
			array('name'=>$rank1.L('YUAN_OF_THE_FOLLOWING'),'where_money'=>array(array('lt',$rank1),array('gt',0), 'and')),
			array('name'=>$rank1.'~'.$rank2.L('YUAN'),'where_money'=>array(array('elt',$rank2),array('gt',$rank1), 'and')),
			...
			array('name'=>$rank4.'~'.$rank5.L('YUAN'),'where_money'=>array(array('elt',$rank5),array('gt',$rank4), 'and'))
		);
