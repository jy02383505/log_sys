...
class LeadsAction extends Action{
		...
		if($this->isPost()){
			...
			if($m_leads->create() && $m_leads_data->create()!==false){
				...
				if($a && $b!==false) {
					...
					alert('error', L('LEADS_MODIFIED_SUCCESSFULLY'), $_SERVER['HTTP_REFERER']);
				}
			}else{
				alert('error', $m_leads->getError().$m_leads_data->getError());
		...
		if($this->isPost()){
			...
			if ('' == $leads_ids) {
				...
				if(($m_leads->where('leads_id in (%s)', $leads_ids)->delete()) && ($m_leads_data->where('leads_id in (%s)', $leads_ids)->delete())){	
					...
					alert('error', L('DELETE FAILED CONTACT THE ADMINISTRATOR'), U('leads/index'));
				}
			}
		} elseif($_GET['id']) {
			...
			if (is_array($leads)) {
				...
				alert('error', L('LEADS_DOES_NOT_EXIST'), U('leads/index'));
			}			
		} else {
		...
		if($_POST['submit']){
			if (isset($_FILES['excel']['size']) && $_FILES['excel']['size'] != null) {
				...
				if (!is_dir($dirname) && !mkdir($dirname, 0777, true)) {
					alert('error', L('ATTACHMENTS TO UPLOAD DIRECTORY CANNOT WRITE'), U('leads/index'));
				}
			...
			if(is_array($info[0]) && !empty($info[0])){
				...
				alert('error', L('UPLOAD FAILED'), U('leads/index'));
			};
			import("ORG.PHPExcel.PHPExcel");
			...
			if ($allRow <= 2) {
				alert('error', L('UPLOAD A FILE WITHOUT A VALID DATA'), U('leads/index'));
			} else {
				$field_list = M('Fields')->where('model = \'leads\'')->order('order_id')->select();
				for($currentRow = 3;$currentRow <= $allRow;$currentRow++){
					...
					if($m_leads->create($data) && $m_leads_data->create($data_date)) {
						...
						if($this->_post('error_handing','intval',0) == 0){
							alert('error', L('ERROR INTRODUCED INTO THE LINE',array($currentRow,$m_leads->getError().$m_leads_data->getError())), U('leads/index'));
						}else{
				...
				alert('success', $error_message.L('IMPORT SUCCESS'), U('leads/index'));
			}
		} else {
			$this->display();
		}
	...
	public function receive(){
		...
		if ($leads_id > 0) {
			...
			if ($a || $b) {
				...
				$content=L('NEW_LEADS_MESSAGE_NOTICE_CONTENT',array(session('name'), $leads['name']));
	...
	public function batchAssign(){
		...
		if($success_leads_name){
			$content = L('NEW_LEADS_MESSAGE_NOTICE_CONTENT' ,array(session('name'), $success_leads_name));
			if($message == 1) {
				sendMessage($owner_role_id,$content,1);
			}
			if($email == 1){
