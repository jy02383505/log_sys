<?php 
// 
class QinjiaAction extends Action {
    
	public function _initialize(){
		$action = array(
			'permission'=>array(),
			'allow'=>array('index','tj','add','edit','accept')
		);
...
//		if (!$_GET['the_month']){$_GET['the_month']=$defaul_month;}
		...
		*/
		...
		if ($the_year){
			$map['jl_kaoqin_qinjia.the_year'] = $the_year;
		}else{
			if(!$the_year){
		...
		if ($user_id){
			...
			$map['jl_kaoqin_qinjia.user_id'] = $user_id;
		}
		
		if ($the_year && $the_month){
			$map['jl_kaoqin_qinjia.the_month'] = $the_month;
		}
		if ($type){
			//$where = "where A.id>0  and A.kqj_user_id=".$kqj_user_id;
			$map['jl_kaoqin_qinjia.type'] = $type;
		}
        ...
        // 数据分页
        $totals = $model->table("jl_kaoqin_qinjia")->where($map)->count();
		//var_dump($map);
		...
		$order = " id DESC";
		$datas = $model->table("jl_kaoqin_qinjia")->where($map)->join(' LEFT JOIN jl_user ON jl_user.user_id = jl_kaoqin_qinjia.user_id')->limit($Page->firstRow.','.$Page->listRows)->order($order)->select();
...
//		var_dump(date("YY-m",'2016-05'));
		...
		*/
		...
		$out_datas = $model->table("jl_kaoqin_qinjia")->where($map_1024)->order("the_day ASC")->select();
//		echo $model->getlastsql();
		//var_dump($out_datas);
		...
		$qj_the_year_hour_count = $model->table("jl_kaoqin_qinjia")->where($map)->sum('hour_num');
	...
	//		$endtime = $config_datas['endtime'];//下班时间
			...
			$datas['hour_num'] = $hour_num;//(float)trim($_REQUEST['hour_num']);//round((strtotime($endtime) - strtotime($starttime))/3600,2);
		...
		//	var_dump($datas);exit;
			$result = $kq_model->table("jl_kaoqin_qinjia")->data($datas)->add();
			//var_dump($result);
			alert('success', '新增请假记录成功',U('Qinjia/index','content='.$_GET['content']));
	...
	//		$datas['starttime'] = trim($_POST['starttime']);//开始时间
	//		$datas['endtime'] = trim($_POST['endtime']);//结束时间
			$datas['hour_num'] = $hour_num;//(float)trim($_REQUEST['hour_num']);//$datas['hour_num'] = round((strtotime($endtime) - strtotime($starttime))/3600,2);
			$datas['the_date'] = $the_date;//年月日
			$datas['the_year'] = date("Y",strtotime($the_date));//年
			$datas['the_month'] = date("Y-m",strtotime($the_date));//年月
			$datas['the_day'] = date("d",strtotime($the_date));//日
	//		$datas['the_time'] = date("H:i:s",strtotime($the_date));//几点几分几秒
			...
			$datas['checked'] = 0;//管理员审核后设为1
		//	var_dump($datas);exit;
			$result = $kq_model->table("jl_kaoqin_qinjia")->save($datas);
			//var_dump($result);
			alert('success', '新增请假记录成功',U('Qinjia/index','content='.$_GET['content']));
...
//			$map['user_id'] = session('user_id');
			$datas = $model->table("jl_kaoqin_qinjia")->where($map)->find();
	...
	public function delete(){
		...
		if (!$id){
			alert('error', '参数错误',U('Qinjia/index','content='.$_GET['content']));	
		}
		if (session("user_id") == 1 || session('position_id') == 2){
			//管理员，可删除任意记录
			$model = new Model();
			$model->table("jl_kaoqin_qinjia")->where("id=".$id)->delete();
			alert('success', '操作成功',U('Qinjia/index','content='.$_GET['content']));
		} else {
			//普通用户，只可删除自己的
			$model = new Model();
			$model->table("jl_kaoqin_qinjia")->where("id=".$id." and user_id=".session("user_id"))->delete();		
			alert('success', '操作成功',U('Qinjia/index','content='.$_GET['content']));	
		}
		exit;
	}
	...
	public function accept(){
		...
		if (session("user_id") == 1 || session('position_id') == 2){
			$model = new Model();
			$datas = $model->table("jl_kaoqin_qinjia")->where("id=".$id)->find();
			if ($datas['checked']){
				$data_accept = 0;
			}else{
				$data_accept = 1;
		...
		//	var_dump($datas);exit;
			...
			$result = $model->table("jl_kaoqin_qinjia")->where("id=".$id)->save($data);
			alert('success', '操作成功',U('Qinjia/index','content='.$_GET['content']));
		}
	}
