			...
			if($m_event->create()){
				...
				if($event_id = $m_event->add()){
					...
					if($_POST['send_email']) {
						...
						$content =L('DEAR',array($to_user['user_name'],$_POST['start_date'],$_POST['end_date'],$_POST['description']));
				...
				if(!session('?admin')){
					foreach($_POST['event_id'] as $key => $value){
						if(!$m_event->where('owner_role_id = %d and event_id = %d', session('role_id'), $value) -> find()){
							alert('error', L('YOU_DO_NOT_HAVE_FULL_AUTHORITY_TO_OPERATE'), $_SERVER['HTTP_REFERER']);
						}
					}
				}
