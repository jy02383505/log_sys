...
class ContractAction extends Action {
	...
	public function add(){
		if($_POST['submit']){
			$contract = M('contract');
			$data['number'] = (trim($_POST['number']) && (trim($_POST['number']) != L('AUTOMATICALLY_GENERATE'))) ?trim($_POST['number']):'5KCRM'.date('Ymd').rand(1000,9999);
			...
			if($contractId = $contract->add($data)){
				...
				if(L('SAVE') == $_POST['submit']){
					alert('success', L('CREATE_A_CONTRACT_SUCCESSFULLY'), U('contract/index'));
				}else{
					alert('success', L('CREATE_A_CONTRACT_SUCCESSFULLY'), $_SERVER['HTTP_REFERER']);
				}
			}else{
				alert('success', L('FAILED_TO_CREATE_THE_CONTRACT'), U('contract/add'));
			}
	...
	public function completeDelete(){
		...
		if ('' == $contract_id) {
			...
			if(M('contract')->where('contract_id in (%s)', $contract_id)->delete()){
				...
				alert('error', L('DELETE FAILED'), U('contract/index'));
			}
		}
	}
			...
			foreach($contract_ids as $k=>$v){
				...
				if(empty($contract_product) && empty($contract_file) && empty($contract_receivables) && empty($contract_payables)){
					...
					if(!empty($contract_product)){
						alert('error', L('DELETE_FAILED_PLEASE_DELETE_UNDER_THE_CONTRACT_OF_PRODUCT_INFORMATION',array($contract['number'])), U('contract/index'));
					}elseif(!empty($contract_file)){
						alert('error', L('DELETE_FAILED_PLEASE_DELETE_UNDER_THE_CONTRACT_OF_PRODUCT_INFORMATION',array($contract['number'])), U('contract/index'));
					}elseif(!empty($contract_receivables)){
						alert('error', L('DELETE_FAILED_PLEASE_DELETE_RECEIVABLES_UNDER_THE_FINANCIAL_INFORMATION_IN_THE_CONTRACT',array($contract['number'])), U('contract/index'));
					}else{
						alert('error', L('DELETE_FAILED_PLEASE_DELETE_RECEIVABLES_UNDER_THE_FINANCIAL_INFORMATION_IN_THE_CONTRACT',array($contract['number'])), U('contract/index'));
					}
				}
