...
class TaskAction extends Action{
		...
		if ($this->isPost()) {
			...
			if ($task = $m_task->create()) {
				...
				if($_POST['owner_role_id_str']){
					...
					$message_content = L('MESSAGE_CONTENT' ,array($_POST['subject'], $creator['user_name'], $creator['department_name'], $creator['role_name'], $_POST['due_date'] ,$_POST['priority'], $_POST['description']));
					...
					if($_POST['submit'] == L('SAVE')) {
						...
						alert('success', L('SUCCESS_ADD'), $_SERVER['HTTP_REFERER']);
					}
				} else {
					alert('error', L('SELECT_TASK_EXECUTOR'),  $_SERVER['HTTP_REFERER']);
				}
			...
			if(session('role_id') == $task['creator_role_id'] || session('?admin')){
				if ($module != '') {
					...
					if ($_POST['module_id']) {
						...
						alert('error', L('SELECT_CORRESPOND_OPTION'), $_SERVER['HTTP_REFERER']);
					}
				}
				...
				if($d_task->where('task_id = %d', $task_id)->save($data)){ 
					...
					alert('error', L('ONLY_STATUS_CAN_CHANGED_MODIFICATION_FAILS'), $_SERVER['HTTP_REFERER']);
				}
			}else{
				alert('error', L('HAVE_NO_PERMISSION_TO_MODIFY'), $_SERVER['HTTP_REFERER']);
			}
	...
	public function delete(){
		...
		if($this->isPost()){
			...
			if ('' == $task_ids) {
				alert('error', L('NOT CHOOSE ANY'), U('task/index'));
			} else {
				if(!session('?admin')){
				...
				if($m_task->where('task_id in (%s)', $task_ids)->save($data)){	
					...
					alert('error', L('DELETE FAILED CONTACT THE ADMINISTRATOR'), U('task/index'));
				}
			}
		} elseif ($_GET['id']) {
			$task = $m_task->where('task_id = %d', $_GET['id'])->find();
			if (is_array($task)) {
				...
				alert('error', L('TASK_NOT_EXIST'), U('leads/index'));
			}			
		} else {
	...
	public function completeDelete(){
		...
		if($this->isPost()){
			$task_ids = is_array($_POST['task_id']) ? implode(',', $_POST['task_id']) : '';
			if ('' == $task_ids) {
				alert('error', L('NOT CHOOSE ANY'), U('leads/index'));
			} else {
				if(!session('?admin')){
					foreach($_POST['task_id'] as $key => $value){
						if(!$m_task->where('owner_role_id = %d and task_id = %d', session('role_id'), $value) -> find()){
	...
	public function analytics(){
		...
		foreach($role_id_array as $v){
			...
			$new_count = $m_task->where(array('is_deleted'=>0,'isclose'=>0,'status'=>L('NOT_START'), 'owner_role_id'=>$v, 'create_date'=>$create_time))->count();
			$late_count = $m_task->where(array('is_deleted'=>0,'isclose'=>0,'status'=>L('DELAY'), 'owner_role_id'=>$v, 'create_date'=>$create_time))->count();
			$deal_count = $m_task->where(array('is_deleted'=>0,'isclose'=>0,'status'=>L('ONGOING'), 'owner_role_id'=>$v, 'create_date'=>$create_time))->count();
			$success_count =  $m_task->where(array('is_deleted'=>0,'isclose'=>0,'status'=>L('COMPLETE'), 'owner_role_id'=>$v, 'create_date'=>$create_time))->count();
