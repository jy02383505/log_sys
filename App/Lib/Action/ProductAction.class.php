	...
	public function add() {
		if($this->isPost()) {
			...
			if (!isset($_POST['name']) || $_POST['name'] == '') {
				alert('error', L('PRODUCT_NAME_CANNOT_BE_EMPTY'), $_SERVER['HTTP_REFERER']);
...
//			$this->categoryList = $categoryList;
	...
	public function delete(){
		...
		if($this->isPost()){
			...
			if ('' == $product_ids) {
				alert('error', L('YOU_HAVE_NOT_CHOOSE_ANY_CONTENT') , U('product/index'));
			} else {
				if (!session('?admin')) {
				...
				if($product_delete && $product_data_delete){
					...
					alert('error', L('DELETE_FAILED_PLEASE_CONTACT_YOUR_ADMINISTRATOR'), U('product/index'));
				}
			...
			if (is_array($product)) {
				...
				alert('error', L('YOU_WANT_TO_DELETE_THE_RECORD_DOES_NOT_EXIST'), U('product/index'));
			}			
		} else {
