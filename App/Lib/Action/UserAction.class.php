		...
		if ($this->isPost()) {
			...
			if ($m_user->create()) {
				...
				if($m_user->save() || $is_update){
					...
					alert('success',L('EDIT_USER_INFO_SUCCESS'),U('user/edit','id='.$user['user_id']));
				}else{
		...
		if ($this->isPost()){
			...
			if($m_config->where('name = "smtp"')->find()){
				...
				if(empty($role_list) && !session('?admin')){
					alert('error', L('YOU_HAVE_NO_PERMISSION_TO_ADD_USER'), U('setting/smtp'));
				}else{
					$this->categoryList = $category->select();
					$this->assign('department_list', $department_list);
					$this->alert = parseAlert();
				...
				alert('error',L('PLEASE_SET_SMTP_FIRST_TO_INVITATION_USER'),U('setting/smtp'));
			}
		}
	}
