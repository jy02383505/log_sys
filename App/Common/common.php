		...
		if ($v['field'] == 'customer_id') {
            ...
            switch ($v['form_type']) {
                ...
                case 'editor' :
                    ...
                    break;
                case 'datetime' :
                    $field_list[$k]['html'] = '<input  onFocus="WdatePicker({dateFmt:\'yyyy-MM-dd\'})" name="'.$v['field'].'" class="span2" id="'.$v['field'].'" type="text" value="'.pregtime($value).'"/> &nbsp;  <span id="'.$v['field'].'Tip" style="color:red;"></span>'.$input_tips;
                    break;
                case 'number' :
...
/**
 * @ atuhor		: Lynn
 * @ function 	: formmat the print_r to debugging the array conveniently
 **/
function println($data, $offset){
	$offset = true;
	if($offset == true){
